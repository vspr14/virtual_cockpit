...
const applySimState = (data) => {
    ...
    if (apIndicator && data.ap_master !== undefined) {
    ...
    if (data.parking_brake !== undefined) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const isInteractiveElement = (el) => {
        ...
        if (el.classList && (el.classList.contains('joy-zone') || el.classList.contains('joy-puck'))) return true;
        return false;
    };

    let multiTouchActive = false;
    let allowedMultiTouch = false;

    const preventZoomStart = (e) => {
        if (e.touches && e.touches.length > 1) {
            multiTouchActive = true;
            allowedMultiTouch = false;
            ...
            allowedMultiTouch = interactiveElements.size === e.touches.length;
            
            if (!allowedMultiTouch) {
                e.preventDefault();
            }
        } else {
    ...
    const preventZoomMove = (e) => {
        if (multiTouchActive && !allowedMultiTouch) {
            e.preventDefault();
        }
    };

    const preventZoomEnd = (e) => {
        if (e.touches && e.touches.length <= 1) {
            multiTouchActive = false;
            allowedMultiTouch = false;
        }
    ...
    document.addEventListener('touchstart', preventZoomStart, { passive: false });
    document.addEventListener('touchmove', preventZoomMove, { passive: false });
    document.addEventListener('touchend', preventZoomEnd, { passive: false });
