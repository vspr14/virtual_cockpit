...
class MobiFlightVariableRequests:
    ...
    def client_data_callback_handler(self, client_data):
        if client_data.dwDefineID in self.sim_vars:
            ...
            # Store every value. (The upstream sample skipped the first 0 to hide the empty slot sent on
            # subscribe, but a variable whose first value was non-zero then lost its first real change to 0,
            # e.g. a switch turned OFF, and stayed stale. register() waits out the empty slot instead.)
            sim_var.initialized = True
            sim_var.float_value = float_value
    ...
    def register(self, variableStrings, settle_s=0.25):
        """Register every variable not registered yet, then give the WASM module time to send the real
        values (the first update is the empty slot, 0). Returns how many were added."""
        added = 0
        for name in variableStrings:
            if name not in self.sim_var_name_to_id:
                self._add(name)
                added += 1
        if added:
            sleep(settle_s)
