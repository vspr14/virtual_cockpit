/* Radar SYS (1 / OFF / 2) and PWS (OFF / AUTO) 3D toggle switches.
   Interaction: tap left/right of a switch to step it one position, or swipe sideways.
   onChange({ which: 'sys' | 'pws', value }) fires only for the switch the user moved;
   applySimState({ sys, pws }) animates to sim positions without firing onChange.
   Indices: sys 0 = "1", 1 = OFF, 2 = "2"; pws 0 = OFF, 1 = AUTO. */
...
export function initRadarPwsSwitches3D(mount, options) {
    ...
    function notify(sw) {
        onChange({ which: sw === sysSwitch ? 'sys' : 'pws', value: sw.getState() });
    }
    ...
    function pickSwitchAt(clientX, clientY) {
        ...
        /* Generous hit area: anywhere in the switch's half of the panel counts. */
        if (raycaster.intersectObjects([sysSwitch.pivotGroup], true).length > 0) return sysSwitch;
        if (raycaster.intersectObjects([pwsSwitch.pivotGroup], true).length > 0) return pwsSwitch;
        const rect = canvas.getBoundingClientRect();
        const sysX = switchScreenX(sysSwitch);
        const pwsX = switchScreenX(pwsSwitch);
        if (sysX == null || pwsX == null) return null;
        const mid = (sysX + pwsX) / 2;
        if (clientY < rect.top || clientY > rect.bottom) return null;
        return clientX < mid ? sysSwitch : pwsSwitch;
    }

    /* Screen x (px) of a switch's pivot. */
    function switchScreenX(sw) {
        const v = new THREE.Vector3();
        sw.pivotGroup.getWorldPosition(v);
        v.project(cam);
        const rect = canvas.getBoundingClientRect();
    ...
    function step(sw, dir) {
        if (sw.step(dir)) notify(sw);
    }
    ...
    function onPointerMove(e) {
        ...
        e.preventDefault();
        if (!swiped && Math.abs(e.clientX - startX) < TAP_SLOP_PX) return;
        swiped = true;
        dragAccum += e.clientX - lastPointerX;
        lastPointerX = e.clientX;
        while (dragAccum >= DRAG_PX_PER_STEP) { step(activeSwitch, 1); dragAccum -= DRAG_PX_PER_STEP; }
        while (dragAccum <= -DRAG_PX_PER_STEP) { step(activeSwitch, -1); dragAccum += DRAG_PX_PER_STEP; }
    }
    ...
    function onPointerUp(e) {
        ...
        if (!swiped) {
            const pivotX = switchScreenX(sw);
            step(sw, e.clientX < pivotX ? -1 : 1);
        }
    }
    ...
    return {
        ...
        /** Move to sim positions (animated) without firing onChange. Pass only the keys to update. */
        applySimState: (st) => {
            if (!st) return;
            if (typeof st.sys === 'number' && st.sys !== sysSwitch.getState()) sysSwitch.moveTo(st.sys);
            if (typeof st.pws === 'number' && st.pws !== pwsSwitch.getState()) pwsSwitch.moveTo(st.pws);
        },
        destroy: () => {
            destroyed = true;
