/* 3D landing gear lever. Drag the wheel knob up/down; onCommit(isDown) fires when it settles in a new
   position. applySimGearDown(down) follows the sim. */
...
export function initGearLever3D(mount, options) {
    ...
    animate();
    return {
        /** Follow the sim's handle position (animated). Ignored while the user is dragging. */
        applySimGearDown(down) {
            if (isDragging) return;
            const d = !!down;
            targetProgress = d ? 0 : 1;
            lastCommittedDown = d;
        }
    };
}
