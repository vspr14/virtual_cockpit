/* Airbus ATC / TCAS pedestal panel (knobs + transponder keypad). Aircraft-agnostic: the page passes
   callbacks for each control and feeds sim state back through the returned setters. Markup lives in
   templates/widgets/tcas_xpdr_panel.html. */
...
export function initTcasXpdrPanel(root, options) {
    if (!root) return {};
    const opt = options || {};
    const out = {};
