...
/* Momentary pushbutton for Fenix-style S_* LVars (HubHop "... Button Press" = 1, "... Release" = 0).
   All pulses go through one queue: press is written, then release, then the next key — so fast typing
   can never overlap or reorder presses and releases (two loose fetches can arrive in either order).
   Tested on the live Fenix A320: with <~100 ms between a release and the next press Fenix misses the
   release and drops the next key; ~220 ms registers every key. */
let lvarPulseChain = Promise.resolve();
let lvarPulsePending = 0;
const LVAR_PULSE_HOLD_MS = 120;
const LVAR_PULSE_GAP_MS = 220;
const queueLvarPulse = (key) => {
    const ui = window.PROFILE?.ui || {};
    const sk = ui.mobiflight_skip_lvar_keys;
    if (Array.isArray(sk) && sk.indexOf(key) >= 0) return;
    if (ui.mobiflight_skip_ped_atc && typeof key === 'string' && key.indexOf('ped_atc_') === 0) return;
    ...
    lvarPulsePending++;
    lvarPulseChain = lvarPulseChain
        .then(function () { return post(1); })
        .then(function () { return wait(LVAR_PULSE_HOLD_MS); })
        .then(function () { return post(0); })
        .then(function () {
            schedulePollLvarsAfterWrite();
            return wait(LVAR_PULSE_GAP_MS);
        })
        .catch(function () {})
        .then(function () { lvarPulsePending = Math.max(0, lvarPulsePending - 1); });
};
...
const applySimState = (data) => {
    ...
    if (tcasKnobSyncRef && Date.now() >= (window.__tcasUserHoldUntil || 0)) {
        ...
        /* Keypad display mirrors Fenix's own entry once every queued key has been sent. */
        const entry = tcasNum(data.xpdr_entry);
        const entryChars = tcasNum(data.xpdr_entry_chars);
        if (
            typeof tcasKnobSyncRef.setXpdrEntry === 'function' &&
            lvarPulsePending === 0 &&
            !Number.isNaN(entry) &&
            !Number.isNaN(entryChars)
        ) {
            tcasKnobSyncRef.setXpdrEntry(entry, entryChars);
        }
    }
    if (
        extLtLightsApi &&
    ...
    if (
        pmdg777LightsApi &&
        typeof pmdg777LightsApi.syncFromSim === 'function' &&
        Date.now() >= (window.__extLtUserHoldUntil || 0)
    ) {
        pmdg777LightsApi.syncFromSim(data);
    }
    ...
    /* Fenix autobrake (Airbus style): the S_MIP_AUTOBRAKE_* LVars are momentary pushbuttons, so the
       button itself never latches. Armed state = blue ON light (I_MIP_AUTOBRAKE_*_L); DECEL = green
       upper light (I_MIP_AUTOBRAKE_*_U). Skip the ON sync briefly after a user press so the optimistic
       UI is not overwritten before Fenix has updated its annunciators. */
    const autobrkHold = Date.now() < (window.__autobrkUserHoldUntil || 0);
    const syncAutobrkPb = function (btnId, upperAnnuncKey, lowerAnnuncKey) {
        const el = document.getElementById(btnId);
        if (!el) return;
        ...
        if (!autobrkHold && lowerEl && typeof data[lowerAnnuncKey] === 'number') {
            lowerEl.classList.toggle('lit', data[lowerAnnuncKey] >= 0.15);
        }
        if (upperEl && typeof data[upperAnnuncKey] === 'number') {
            upperEl.classList.toggle('lit', data[upperAnnuncKey] >= 0.15);
        }
    };
    syncAutobrkPb('gearPb1', 'i_mip_autobrake_lo_u', 'i_mip_autobrake_lo_l');
    syncAutobrkPb('gearPb2', 'i_mip_autobrake_med_u', 'i_mip_autobrake_med_l');
    syncAutobrkPb('gearPb3', 'i_mip_autobrake_max_u', 'i_mip_autobrake_max_l');
};
...
const CAMERA_SEND_DEBOUNCE_MS = 280;
let _lastCameraSendMs = 0;

const sendCamera = (camId) => {
    if (camId == null) return;
    const now = Date.now();
    if (now - _lastCameraSendMs < CAMERA_SEND_DEBOUNCE_MS) return;
    _lastCameraSendMs = now;
    selectedCamId = camId;
...
const bindButtonTouch = (btn) => {
    ...
    btn.addEventListener('click', (e) => {
        if (
            btn.__vcSuppressGhostClickUntil &&
            Date.now() < btn.__vcSuppressGhostClickUntil &&
            e.detail > 0
        ) {
            e.preventDefault();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderOfp = (pdfUrl, metars) => {
        ...
        if (centerZone) centerZone.classList.add('ofp-mode');
        setNavActive(ofpBtn);
    };
    ...
    const showCams = () => {
        ...
        if (centerZone) centerZone.classList.remove('ofp-mode');
        setNavActive(camBtn);
    };
    ...
    if (revBtn) bindButtonTouch(revBtn);
    ...
    if (gearLever && !document.getElementById('gear-3d-mount')) {
        gearIsDown = false;
        gearLever.classList.remove('gear-down');
        gearLever.classList.add('gear-up');
    ...
    if (tcasXpdrRoot && typeof window.initTcasXpdrPanel === 'function') {
        const pulseAtc = function (key) { queueLvarPulse(key); };
    ...
    const pmdg777LightsRoot = document.getElementById('pmdg777LightsRoot');
    if (pmdg777LightsRoot) {
        import('/static/js/pmdg_777_lights_panel.js')
            .then((mod) => {
                if (typeof mod.attachPmdg777LightsPanel !== 'function') return;
                return mod.attachPmdg777LightsPanel(pmdg777LightsRoot, {
                    emit: (key, val) => setLvar(key, val),
                    onUserInteraction: bumpExtLtUserHold,
                });
            })
            .then((api) => {
                pmdg777LightsApi = api;
                applyCachedLvarPollIfAny();
            })
            .catch(() => {});
    }
    ...
    /* Airbus autobrake pushbuttons (Fenix): each press is a momentary 1 -> 0 on S_MIP_AUTOBRAKE_*
       (HubHop "FNX320 MainPanel Autobrake ... Button Press/Release"). Fenix does the arming logic:
       press an unarmed mode -> arms it (and disarms any other); press the armed mode -> disarms.
       The UI mirrors that logic immediately, then follows the sim's ON annunciators. */
    const pulseAutobrkLvar = (key) => { queueLvarPulse(key); };
    ...
    const bindAutobrkExclusiveGroup = (ids) => {
        ...
        const onLight = (b) => b && (b.querySelector('.autobrake-annunc-lower') || b.querySelector('.pack-apu-start-on'));
        buttons.forEach(function (btn, idx) {
            ...
            btn.addEventListener('click', function (e) {
                if (e && typeof e.stopPropagation === 'function') e.stopPropagation();
                const target = this;
                const light = onLight(target);
                const wasArmed = !!(light && light.classList.contains('lit'));
                buttons.forEach(function (b, i) {
                    const l = onLight(b);
                    if (l) l.classList.toggle('lit', !wasArmed && i === idx);
                });
                window.__autobrkUserHoldUntil = Date.now() + 1500;
                target.classList.remove('pack-anim-press');
                void target.offsetWidth;
                target.classList.add('pack-anim-press');
                setTimeout(function () { target.classList.remove('pack-anim-press'); }, 320);
                if (useMfMip()) pulseAutobrkLvar(keys[idx]);
            });
        });
    };
    ...
    const saveState = () => {
        ...
        if (!profileName || !f || !sSlider) return;
        const state = {
            profile: profileName,
            flaps: parseFloat(f.value),
            throttle: tSlider ? parseFloat(tSlider.value) : 0,
